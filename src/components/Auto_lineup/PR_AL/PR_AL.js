...
class PR_AL extends Component {
  ...
  componentDidMount = async () => {
    ...
    let mc_list_data = await httpClient.post(server.AL_MC_LIST);
    await this.setState({
  ...
  data_by_total = async () => {
    let day_ttl = await httpClient.post(server.DAILY_TOTAL_AL, {
    ...
    await this.setState({
      ...
      all_options: {
        title: {
          text: "AUTO LINE UP TOTAL OUTPUT",
          align: 'center',
          style: {
            fontSize: '30px',
            fontWeight: 'bold',
  ...
  data_by_mc = async () => {
    let mc_ttl = await httpClient.post(server.DAILY_OUT_MC_AL, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      process: this.state.selected_process,
      selected_mc: this.state.selected_mc,
    ...
    await this.setState({
      ...
      options: {
        title: {
          text: 'AUTO LINE UP OUTPUT BY MACHINE',
          align: 'center',
          style: {
            fontSize: '30px',
            fontWeight: 'bold',
  ...
  month_data_output = async () => {
    let day_ttl = await httpClient.post(server.MONTHLY_total_outputAL, {
    ...
    await this.setState({
      ...
      monthly_output_options: {
        title: {
          text: "AUTO LINE UP MONTHLY OUTPUT",
          align: 'center',
          style: {
            fontSize: '30px',
            fontWeight: 'bold',
  ...
  ct_day = async () => {
    let ct_day = await httpClient.post(server.daily_CT_AL, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    });
  ...
  ct_month = async () => {
    let ct_month = await httpClient.post(server.month_CT_AL, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    });
    console.log(ct_month.data);
    var label_ct = [];
    var ct_data = [];
    var ct_target = [];
    for (let x = 0; x < ct_month.data.result.length; x++) {
      label_ct.push(ct_month.data.result[x].month);
      ct_data.push(ct_month.data.result[x].ct);
    }
    await this.setState({
      monthly_ct_series: [
        {
    ...
    return (<div className="content-wrapper">
      <section className="content-header">
        <div className="container-fluid">
          <h2 className="text-center" style={{ fontWeight: 'bold' }} >AUTO LINE UP PRODUCTION RESULT </h2>
        </div>
      </section>
