...
// IRH result
class PR_Grinding extends Component {
  ...
  componentDidMount = async () => {
    let mc_list_data = await httpClient.get(server.list_machine_GD);
    await this.setState({
  ...
  data_by_total = async () => {
    let day_ttl = await httpClient.post(server.DAILY_TOTAL_GD, {
  ...
  data_by_mc = async () => {
    let mc_ttl = await httpClient.post(server.IRH_DAILY_OUT_MC, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      process: this.state.selected_process,
      selected_mc: this.state.selected_mc,
  ...
  month_data_output = async () => {
    let day_ttl = await httpClient.post(server.MONTHLY_total_output_IRH, {
    ...
    // CT by day
    let ct_day = await httpClient.post(server.daily_CT_IRH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    ...
    let UTL_day = await httpClient.post(server.daily_UTL_IRH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    });
    //console.log(UTL_day.data);
    var label_utl = [];
    var utl_data = [];
    for (let a = 0; a < UTL_day.data.result.length; a++) {
      label_utl.push(UTL_day.data.result[a].registered_at);
      utl_data.push(UTL_day.data.result[a].utl);
    }
    await this.setState({
      all_series_utl: [
        {
  ...
  month_CTandUTL = async () => {
    let day_ct = await httpClient.post(server.CT_month_IRH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    })
    ...
    let day_UTL = await httpClient.post(server.UTL_month_IRH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    })
    ...
    return (<div className="content-wrapper">
      <section className="content-header">
        <div className="container-fluid">
          <h2 className="text-center" style={{ fontWeight: 'bold' }} >IRH GRINDING RESULT </h2>
        </div>
      </section>
