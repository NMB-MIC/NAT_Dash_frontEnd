...
class Dash_AN extends Component {
  constructor(props) {
    super(props);
  ...
  show_chart_output = async () => {
    // output chart
    let output_data = await httpClient.post(server.AN_OUTPUT, {
  ...
  cycle_time = async () => {
    let mc_ttl = await httpClient.post(server.AN_CT, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
  ...
  getCT_byHour = async () => {
    let CT = await httpClient.post(server.AN_ct_byHour, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
    });
      ...
      <div className="content-wrapper">
        <section className="content-header">
          <div className="container-fluid">
            <h2 className="text-center" style={{ fontWeight: 'bold' }} > AUTO NOISE DASHBOARD </h2>
          </div>
        </section>
        <div className="row">
