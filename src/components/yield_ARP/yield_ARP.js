...
class Yield_ARP extends Component {
  ...
  total_yield = async () => {
    let get_data = await httpClient.post(server.TTL_yield_ARP, {
  ...
  total_yield_data = async () => {
    let total_yield_data = await httpClient.post(server.TTL_Yield_data_ARP, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      process: this.state.selected_process,
      //selected_shift: this.state.selected_shift,
  ...
  NG_ratio = async () => {
    let ng_ratio_result = await httpClient.post(server.NG_ratio_ARP, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      // process: this.state.selected_process,
      // selected_shift: this.state.selected_shift,
    ...
    for (let k = 0; k < ng_ratio_result.data.result.length; k++) {
      ...
      NG_plus.push(ng_ratio_result.data.result[k].NG_plus);
      NG_minus.push(ng_ratio_result.data.result[k].NG_minus);
      ...
      <div className="content-wrapper" style={{ minHeight: 1662 }}>
        <section className="content-header">
          <div className="container-fluid">
            <h2 className="text-center" style={{ fontWeight: 'bold' }} > ARP YIELD AND NG RATIO  </h2>
          </div>
        </section>
