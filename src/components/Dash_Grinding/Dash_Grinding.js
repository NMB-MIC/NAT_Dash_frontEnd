...
class Dash_Grinding extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount = async () => {
    ...
    let mc_list_data = await httpClient.post(server.GD_MC_LIST);
    await this.setState({
  ...
  show_chart_output = async () => {
    // output chart
    let output_data = await httpClient.post(server.GD_OUTPUT, {
  ...
  cycle_time = async () => {
    let mc_ttl = await httpClient.post(server.GD_CT, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
  ...
  getCT_byHour = async () => {
    let CT = await httpClient.post(server.GD_ct_byHour, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
    });
    let UTL = await httpClient.post(server.GD_UTL, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
    })
      ...
      <div className="content-wrapper">
        <section className="content-header">
          <div className="container-fluid">
            <h2 className="text-center" style={{ fontWeight: 'bold' }} > IRH GRINDING DASHBOARD </h2>
          </div>
        </section>
        <div className="row" >
