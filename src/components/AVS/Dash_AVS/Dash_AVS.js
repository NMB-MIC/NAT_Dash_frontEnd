...
class Dash_AVS extends Component {
  ...
  componentDidMount = async () => {
    let mc_list_data = await httpClient.post(server.list_mc_AVS);
    await this.setState({
  ...
  show_chart_output = async () => {
    // output chart
    let output_data = await httpClient.post(server.AVS_OUTPUT, {
  ...
  cycle_time = async () => {
    let mc_ttl = await httpClient.post(server.AVS_CT, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
  ...
  getCT_byHour = async () => {
    let CT = await httpClient.post(server.AVS_ct_byHour, {
      date: this.state.date_start,
      machine: this.state.selected_machine,
    });
    // let UTL = await httpClient.post(server.AN_UTL, {
    //   date: this.state.date_start,
    //   machine: this.state.selected_machine,
    // })
      ...
      <div className="content-wrapper">
        <section className="content-header">
          <div className="container-fluid">
            <h2 className="text-center" style={{ fontWeight: 'bold' }} > AVS DASHBOARD </h2>
          </div>
        </section>
        <div className="row">
