...
class PR_ORH extends Component {
  ...
  componentDidMount = async () => {
    this.getDate();
    let mc_list_data = await httpClient.post(server.list_machine_ORH);
    await this.setState({
  ...
  data_by_total = async () => {
    let day_ttl = await httpClient.post(server.DAILY_TOTAL_ORH,{
  ...
  data_by_mc = async () => {
    let mc_ttl = await httpClient.post(server.ORH_DAILY_OUT_MC, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      process: this.state.selected_process,
      selected_mc: this.state.selected_mc,
  ...
  month_data_output = async () => {
    let day_ttl = await httpClient.post(server.MONTHLY_total_output_ORH, {
  ...
  get_ct_data = async () => {
    let ct_day = await httpClient.post(server.daily_CT_ORH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    ...
    let day_ct = await httpClient.post(server.CT_month_ORH, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    })
    ...
    return (<div className="content-wrapper">
      <section className="content-header">
        <div className="container-fluid">
          <h2 className="text-center" style={{ fontWeight: 'bold' }} >ORH GRINDING RESULT </h2>
        </div>
      </section>
