...
class PR_ARP extends Component {
  ...
  get_ARPmc = async () => {
    let mc_list_mc = await httpClient.post(server.list_mc_ARP);
    await this.setState({
      list_machine_ARP: mc_list_mc.data.result,
    });
    console.log(mc_list_mc.data.result);
  }
  renderTableRow = () => {
    try {
      if (this.state.list_machine_ARP !== null) {
        const myResult = this.state.list_machine_ARP;
  ...
  data_by_total = async () => {
    let day_ttl = await httpClient.post(server.DAILY_TOTAL_ARP, {
  ...
  data_by_mc = async () => {
    ...
    let mc_ttl = await httpClient.post(server.DAILY_OUT_MC_ARP, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      process: this.state.selected_process,
      selected_mc: this.state.selected_mc,
  ...
  month_data_output = async () => {
    let day_ttl = await httpClient.post(server.MONTHLY_total_outputARP, {
  ...
  ct_day = async()=> {
    let ct_day = await httpClient.post(server.daily_CT_ARP, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    }); 
...
CT_month  = async()=> {
    let ct_day = await httpClient.post(server.month_CT_ARP, {
      date_start: this.state.date_start,
      date_end: this.state.date_end,
      selected_mc: this.state.selected_mc,
    }); 
      ...
      <div className="content-wrapper">
        ...
        <section className="content-header">
          <div className="container-fluid">
            <h2 className="text-center" style={{ fontWeight: 'bold' }} >  ARP PRODUCTION RESULT </h2>
          </div>
        </section>
